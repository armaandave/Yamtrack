...
enum GoodreadsImportJobPhase: Equatable {
    case idle
...
extension Notification.Name {
    static let goodreadsImportDidSucceed = Notification.Name("goodreadsImportDidSucceed")
}

@MainActor
@Observable
final class GoodreadsImportCoordinator {
    var phase: GoodreadsImportJobPhase = .idle
    ...
    private func runImport(fileURL: URL, mode: ImportMode) async {
        guard fileURL.pathExtension.lowercased() == "csv" else {
            phase = .failed(message: "Please upload the .csv export from Goodreads, not a ZIP file.")
            return
        }
        ...
        do {
            ...
            let response = try await importRepository.queueGoodreadsImport(
                fileData: file.data,
                fileName: file.name,
                mode: mode,
    ...
    private func handle(task: ImportTaskStatus, taskId: String, startedAt: Date) -> Bool {
        ...
        case "SUCCESS":
            clearPersistedJob()
            phase = .succeeded(message: task.result?.trimmedNonEmpty ?? "Goodreads import complete.")
            NotificationCenter.default.post(
                name: .goodreadsImportDidSucceed,
                object: self,
        ...
        case "FAILURE":
            clearPersistedJob()
            phase = .failed(message: task.result?.trimmedNonEmpty ?? "Goodreads import failed.")
            return true
        default:
            if Date().timeIntervalSince(startedAt) >= timeout {
                phase = .failed(message: Self.timeoutMessage)
    ...
    private nonisolated static func readFile(_ url: URL) async throws -> (data: Data, name: String, size: Int64?) {
        try await Task.detached(priority: .userInitiated) {
            ...
            let fileName = trimmedFileName.isEmpty ? "goodreads_library_export.csv" : trimmedFileName
    ...
    private enum Keys {
        static let taskId = "goodreadsImport.taskId"
        static let mode = "goodreadsImport.mode"
        static let startedAt = "goodreadsImport.startedAt"
    }
}
